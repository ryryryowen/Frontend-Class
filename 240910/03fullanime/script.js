...
const sec3 = () => {
  ...
  anime({
    ...
    fill: [{ value: "#667eea" }, { value: "#763ba2" }, { value: "#66a6ff" }],
    duration: 3000,
...
const next = () => {
  ...
  let i = parseInt(crtSlide.dataset.index); // 인덱스를 정수로 변환
  s_reset();
  i++;
  if (i >= s_slider.length) i = 0; // 인덱스가 슬라이드의 수를 넘을 경우 0으로 리셋
  s_slider[i].classList.add("on");
  s_icons[i].classList.add("active");
  bg.style.backgroundImage = `url("./img/sec2_bg_${i}.png")`; // 배경 이미지 변경
};

s_right.addEventListener("click", next);

let crtSlide = bg.querySelector(".sec2_slider li.on"); // '.on'으로 수정
...
const prev = () => {
  ...
  s_reset();
  i--;
  if (i < 0) i = s_slider.length - 1; // 인덱스가 음수일 경우 마지막 슬라이드로
  s_slider[i].classList.add("on");
  s_icons[i].classList.add("active");
  bg.style.backgroundImage = `url("./img/sec2_bg_${i}.png")`; // 배경 이미지 변경
};
